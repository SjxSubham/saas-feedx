...
const Page = async ({ params }: { 
    params: { projectId: string
        ...
        return  (
            <div>
                <div>
                    <Link href="/dashboard" className="flex items-center text-indigo-700 mb-3 w-fit"><ChevronLeft className="h-5 w-5 mr-1" /><span className="text-lg">Back to all Projects</span></Link>
                </div>
            <div className="flex justify-between items-start">
            <div className="proj-info">
                <div >
                <h1 className="text-3xl font-bold mb-3 ">{project.name}</h1>
                <h2 className="text-primary-background text-xl ">{project.description}</h2>
                </div>
                ...
                <Code className="h-5 w-5 mr-1"/><span className="text-lg">Embed Code</span></Link></div>
            </div>
            </div>
            <div>
